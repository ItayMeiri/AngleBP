...
from OOD.approxgrad_final3 import ApproxGrad
...
for in_dataset, out_dataset, model_name, model_type, use_pretrained, feature_extract in model_data:
    ...
    _, in_dist_test = utils.get_datasets(in_dataset, in_transformation)

    # Test accuracy on in_dist_test

    _, out_dist_test = utils.get_datasets(out_dataset, out_transformation)
    ...
    for method in methods:
        ...
        if method == "gradneighbors":
            ...
            combined_dataloader = torch.utils.data.DataLoader(combined_dataset, batch_size=125, shuffle=True)
            ...
            dots = signed_gradients@unit_vector.T
            ...
            if mag_auroc > best_auroc:
                ...
                print("Best weights, func, feature:", best_weights, best_func, best_feature)
                _, comb_fpr, comb_tpr, thresholds = get_auroc(features=feature1, weights=best_weights, y_binary=y_binary, func=best_func, return_fpr_tpr=True)
                # Get best tpr for fpr < 5%
                best_tpr = comb_tpr[np.where(comb_fpr < 0.05)][-1]
                # fpr at best tpr
                best_fpr = comb_fpr[np.where(comb_tpr == best_tpr)][0]

                print("AUROC, TPR, FPR:", best_auroc, best_tpr, best_fpr)
                print("VS")
                mag_fpr, mag_tpr, _ = metrics.roc_curve(y_binary, magnitudes)
                best_mag_tpr = mag_tpr[np.where(mag_fpr < 0.05)][-1]
                best_mag_fpr = mag_tpr[np.where(mag_fpr == mag_tpr)][0]
                roc_auc = metrics.auc(mag_fpr, mag_tpr)
                print("Mag AUROC, TPR, FPR:", roc_auc, best_mag_tpr, best_mag_fpr)

                # Find largest gap between mag_tpr and comb_tpr for fpr < 10%
                largest_gap = 0
                ...
                for i in range(len(comb_fpr)):
                    if comb_fpr[i] <= 0.1 and comb_tpr[i] > mag_tpr[i] and mag_fpr[i] > comb_fpr[i]:
                ...
                # best_tpr = tpr[fpr < 0.05][-1]

            # Generate all combinations of weights for the subset
            mags = np.zeros(20000)

            # Per-class magnitudes

                # Binary search for best weight

                # Deal with ith class ideal weights



            # weight_combinations = product(np.arange(0.9, 1, 0.1), repeat=len(np.unique(y_preds)))
            # start = time.time()
            # for weights in weight_combinations:
            #     for i, weight in enumerate(weights):
            #         mags[y_preds == i] = magnitudes[y_preds == i] * weight
            #     auroc = calculate_auroc(y_binary, mags)
            #     if auroc > best_auroc:
            #         best_auroc = auroc
            #         best_weights = weights
            #     if time.time() - start > 30:
            ...
            #         print("current weights:", weights)
            #
            # if best_auroc > mag_auroc:
            #     print("Best AUROC for per-class magnitudes:", best_auroc)
            #     print("Best weights:", best_weights)
            # else:
            #     print("Per-class magnitudes failed to beat magnitudes")
            #     print("Best AUROC for per-class magnitudes:", best_auroc)
